...
func (server *Server) AddUserRouters() {
	...
	server.Router.HandleFunc("/users", middlewares.SetMiddlewareJSON(server.Handler.GetUsers)).Methods("GET", "OPTIONS")
	server.Router.HandleFunc("/getprofile", middlewares.SetMiddlewareJSON(middlewares.SetMiddlewareAuthentication(server.Handler.GetProfile))).Methods("GET", "OPTIONS")
